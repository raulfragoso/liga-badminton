...
export const LevelListView: React.FC<LevelListViewProps> = ({
  ...
  return (
    <div className="w-full space-y-6 animate-fadeIn">
      {/* Cabeçalho da Aba */}
      <div className="glass-panel p-6 rounded-2xl border border-slate-800 bg-slate-900/90 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <div className="p-2 bg-orange-500/10 rounded-xl border border-orange-500/30 text-orange-400">
              <Layers className="w-5 h-5" />
            </div>
            <h2 className="text-xl font-bold text-white">Classificação por Níveis da Pirâmide</h2>
          </div>
        ...
        <div className="w-full md:w-auto flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-orange-500 font-semibold"
          >
          ...
          {/* Botão Alternar Ordem (Topo -> Base vs Base -> Topo) */}
          <button
            onClick={() => setSortOrder(prev => prev === 'desc' ? 'asc' : 'desc')}
            className="px-3 py-2 bg-slate-950 hover:bg-slate-800 border border-slate-800 rounded-xl text-xs font-semibold text-slate-300 flex items-center gap-1.5 transition-colors"
